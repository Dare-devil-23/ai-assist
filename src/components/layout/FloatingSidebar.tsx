...
export function FloatingSidebar({ position, isOpen, onToggle, children }: FloatingSidebarProps) {
  ...
  const getPositionStyles = () => {
    const baseClasses = "fixed bottom-2 h-10 w-10 md:w-auto md:h-auto bg-none md:top-20 md:bottom-3 z-20 transition-all duration-300 ease-in-out flex flex-col md:bg-white md:border-slate-200 md:rounded-full md:rounded-lg md:shadow-lg";
  ...
  const renderToggleButton = () => {
    ...
    const buttonClass = cn(
      "absolute top-1  md:top-2  bg-white rounded-full p-1 shadow-sm border border-slate-100 z-50",
      position === "left" ? isOpen ? "md:-right-3 -right-1" : "md:-right-3 right-5" : isOpen ? "md:-left-3 -left-1" : "md:-left-3 left-5"
    );
    ...
    <>
      ...
      <aside className={getPositionStyles()}>
        ...
        {position === "left" ? (
          ...
          // Right sidebar
          isOpen ? (
            <div className={`w-64 border-r border-slate-200 rounded-xl bg-[#fbfbfa] overflow-y-auto transition-all duration-300 ease-in-out transform ${isOpen ? 'translate-x-0 inset-y-0 right-1 mt-[70px] md:mt-0 mb-[5px]' : 'translate-x-full'} lg:static fixed z-20 pt-5 notion-sidebar`}>
